...
```bash
git clone https://github.com/jaemin-jo/changupmate.git
cd changupmate/mcp-server
npm install
```
...
```bash
claude mcp add changupmate -- node /절대경로/changupmate/mcp-server/index.js
```
...
```json
{
  "mcpServers": {
    "changupmate": {
      "command": "node",
      "args": ["/절대경로/changupmate/mcp-server/index.js"]
    }
  }
}
```
